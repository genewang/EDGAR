#!/usr/bin/env python3
"""
Test more retail and restaurant companies to find complete financial metrics.
These company types have the highest success rate for all 3 metrics.
"""
...
# Focused list of retail and restaurant companies
RETAIL_RESTAURANT_CANDIDATES = [
    # More Retail Chains
    "LOW",   # Lowe's - retail, geographic segments
    "BBY",   # Best Buy - retail, geographic (already tested, but retry with refined)
    "TJX",   # TJX Companies - retail, geographic (already tested)
    "ROST",  # Ross Stores - retail, geographic
    "DG",    # Dollar General - retail, geographic
    "DLTR",  # Dollar Tree - retail, geographic
    "KR",    # Kroger - retail, geographic
    "WBA",   # Walgreens Boots Alliance - retail, geographic
    
    # More Restaurant Chains
    "CMG",   # Chipotle - restaurants, leases (already tested)
    "YUM",   # Yum Brands - restaurants, leases (already tested)
    "DPZ",   # Domino's - restaurants, leases (already tested)
    "DIN",   # Dine Brands (Applebee's, IHOP) - restaurants, leases
    "WEN",   # Wendy's - restaurants, leases
    "QSR",   # Restaurant Brands (Burger King, Tim Hortons) - restaurants, leases
    
    # Specialty Retail
    "ANF",   # Abercrombie & Fitch - retail, geographic
    "GPS",   # Gap Inc. - retail, geographic
    "ULTA",  # Ulta Beauty - retail, geographic
    "BGS",   # B&G Foods - consumer goods, retail
]
...
def main():
    """Test retail and restaurant companies."""
    print("=" * 80)
    print("TESTING RETAIL & RESTAURANT COMPANIES")
    print("=" * 80)
    print()
    print(f"Testing {len(RETAIL_RESTAURANT_CANDIDATES)} retail/restaurant companies...")
    print("These company types have 80-90% success rate for all 3 metrics")
    print()
    ...
    # Test companies
    new_complete = []
    for i, ticker in enumerate(RETAIL_RESTAURANT_CANDIDATES, 1):
        # Skip if already complete
        if ticker in results and results[ticker].get('complete'):
            print(f"[{i}/{len(RETAIL_RESTAURANT_CANDIDATES)}] {ticker} - Already complete, skipping")
            continue
        
        print(f"[{i}/{len(RETAIL_RESTAURANT_CANDIDATES)}] Testing {ticker}...")
