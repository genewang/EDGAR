#!/usr/bin/env python3
"""
Test fintech companies to find complete financial metrics.
Fintech companies often have clear geographic segments and may have leases/depreciation.
"""
...
# Fintech companies - payment processors, financial services, fintech platforms
FINTECH_CANDIDATES = [
    # Payment Processors (already have V, but test more)
    "MA",    # Mastercard - payment processing, geographic segments
    "PYPL",  # PayPal - digital payments, geographic segments
    "SQ",    # Block (Square) - payment processing, geographic
    "FIS",   # FIS - financial services technology
    "FISV",  # Fiserv - financial services technology
    
    # Financial Services Tech
    "ADP",   # ADP - payroll services, geographic segments
    "INTU",  # Intuit - financial software (QuickBooks, TurboTax), geographic
    "BKNG",  # Booking Holdings - online travel, geographic segments
    "EXPE",  # Expedia - online travel, geographic segments
    
    # Digital Financial Platforms
    "COIN",  # Coinbase - cryptocurrency exchange, geographic
    "HOOD",  # Robinhood - trading platform, geographic
    "SOFI",  # SoFi - digital financial services, geographic
    
    # Credit/Debit Card Networks
    "AXP",   # American Express - credit card, geographic segments
    "DFS",   # Discover Financial - credit card, geographic
    
    # Financial Data/Software
    "MSCI",  # MSCI - financial data and analytics, geographic
    "SPGI",  # S&P Global - financial data, geographic segments
    "MCO",   # Moody's - credit ratings, geographic segments
    
    # Other Fintech
    "NU",    # Nu Holdings - digital banking (Brazil-based but may have US segment)
    "LC",    # LendingClub - online lending platform
]
...
def main():
    """Test fintech companies."""
    print("=" * 80)
    print("TESTING FINTECH COMPANIES")
    print("=" * 80)
    print()
    print(f"Testing {len(FINTECH_CANDIDATES)} fintech companies...")
    print("Fintech companies often have:")
    print("  - Clear geographic segment reporting")
    print("  - Office leases (lease liabilities)")
    print("  - Depreciation from technology infrastructure")
    print()
    ...
    # Test companies
    new_complete = []
    for i, ticker in enumerate(FINTECH_CANDIDATES, 1):
        # Skip if already complete
        if ticker in results and results[ticker].get('complete'):
            print(f"[{i}/{len(FINTECH_CANDIDATES)}] {ticker} - Already complete, skipping")
            continue
        
        print(f"[{i}/{len(FINTECH_CANDIDATES)}] Testing {ticker}...")
