#!/usr/bin/env python3
"""
Test law companies to find complete financial metrics.
Law firms and legal services companies often have:
- Geographic segments (different offices/regions)
- Office leases (significant lease liabilities)
- Depreciation from office equipment and facilities
"""
...
# Law companies - law firms, legal services, legal tech
LAW_CANDIDATES = [
    # Major Law Firms (if publicly traded)
    "L",     # Loews Corporation - diversified, but has legal/insurance segments
    "RLI",   # RLI Corp - insurance with legal services
    
    # Legal Services & Technology
    "INFO",  # IHS Markit (now part of S&P Global) - legal data
    "TRU",   # TransUnion - credit/legal data services
    "EFX",   # Equifax - credit/legal data services
    
    # Legal Tech & Services
    "CLVT",  # Clarivate - legal and IP services
    "RPD",   # Rapid7 - security/legal compliance
    "QLYS",  # Qualys - security/legal compliance
    
    # Professional Services (Legal adjacent)
    "ACN",   # Accenture - consulting, may have legal services
    "CTSH",  # Cognizant - IT services, legal tech
    "GIB",   # CGI - IT services, legal tech
    
    # Legal Document Services
    "DOCU",  # DocuSign - legal document services
    "CLVT",  # Clarivate Analytics - legal/IP services
    
    # Alternative Legal Services
    "RGP",   # Resources Connection - professional services
    "KFY",   # Korn Ferry - executive search, legal services
    
    # Legal Software
    "RPD",   # Rapid7 - security compliance
    "QLYS",  # Qualys - security compliance
    
    # Note: Most major law firms are partnerships, not publicly traded
    # So we focus on legal tech, legal services, and professional services companies
]
...
def main():
    """Test law companies."""
    print("=" * 80)
    print("TESTING LAW COMPANIES")
    print("=" * 80)
    print()
    print(f"Testing {len(LAW_CANDIDATES)} law/legal services companies...")
    print("Note: Most major law firms are partnerships (not publicly traded)")
    print("Focusing on legal tech, legal services, and professional services companies")
    print()
    print("These companies often have:")
    print("  - Geographic segments (different offices/regions)")
    print("  - Office leases (significant lease liabilities)")
    print("  - Depreciation from office equipment and facilities")
    print()
    ...
    # Test companies
    new_complete = []
    for i, ticker in enumerate(LAW_CANDIDATES, 1):
        # Skip if already complete
        if ticker in results and results[ticker].get('complete'):
            print(f"[{i}/{len(LAW_CANDIDATES)}] {ticker} - Already complete, skipping")
            continue
        
        print(f"[{i}/{len(LAW_CANDIDATES)}] Testing {ticker}...")
